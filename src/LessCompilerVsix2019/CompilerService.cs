    ...
    static class CompilerService
    {
        ...
        public static void Process(string configFile, IEnumerable<Config> configs = null, bool force = false)
        {
            ThreadPool.QueueUserWorkItem((o) =>
            {
                try
                {
                    ...
                    {
                        LessCompilerInitPackage.StatusText("Done compiling");
                    }
                }
                catch (Exception ex) when (ex is FileNotFoundException || ex is DirectoryNotFoundException)
                {
                    ...
                    Logger.Log(message);
                    LessCompilerInitPackage.StatusText(message);
                ...
                catch (Exception ex)
                {
                    ...
                    _dte.StatusBar.Progress(false);
                    LessCompilerInitPackage.StatusText($"{Constants.VSIX_NAME} couldn't compile successfully");
                }
                finally
                {
                    _dte.StatusBar.Progress(false);
        ...
        public static void SourceFileChanged(string configFile, string sourceFile)
        {
            ThreadPool.QueueUserWorkItem((o) =>
            {
                try
                {
                    LessCompilerInitPackage.StatusText($"Compiling...");
                ...
                catch (FileNotFoundException ex)
                {
                    ...
                    LessCompilerInitPackage.StatusText($"{Constants.VSIX_NAME} could not find \"{ex.FileName}\"");
                }
                catch (Exception ex)
                {
                    Logger.Log(ex);
        ...
        private static void ShowError(string configFile)
        {
            ...
            if (File.Exists(configFile))
                LessCompilerPackage._dte.ItemOperations.OpenFile(configFile);
        }
