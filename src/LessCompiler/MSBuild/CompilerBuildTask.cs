...
namespace LessCompiler
{
    ...
    public class CompilerBuildTask : Task
    {
        ...
        public override bool Execute()
        {
            ...
            Log.LogMessage(MessageImportance.High, Environment.NewLine + "LessCompiler: Begin compiling " + configFile.Name);
            ...
            CompilerService.Initializing += (s, e) => { Log.LogMessage(MessageImportance.High, "LessCompiler installing updated versions of the compilers..."); };
            ...
            try
            {
                ...
                foreach (CompilerResult result in results)
                {
                    if (result.HasErrors)
                    {
                        ...
                        {
                            Log.LogError("LessCompiler", "0", "", error.FileName, error.LineNumber, error.ColumnNumber, error.LineNumber, error.ColumnNumber, error.Message, null);
                        }
                    }
                }

                Log.LogMessage(MessageImportance.High, "LessCompiler: Done compiling " + configFile.Name);
                return isSuccessful;
            }
            catch (Exception ex)
            {
